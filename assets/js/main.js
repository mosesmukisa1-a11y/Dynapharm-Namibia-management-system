        ...
        function clearPortalContent() {
            ...
            if (portalContentSubtitle) portalContentSubtitle.textContent = 'Sign in to load your workspace below.';
        }
        ...
        function renderAuthState(user) {
            ...
            if (user) {
                ...
                if (accountMenuBranch) accountMenuBranch.textContent = user.branch ? `Primary branch • ${toTitleCase(user.branch)}` : '';
        ...
        function getRoleRedirect(role, overridePortal) {
            ...
            const portalRoutes = {
                ...
                dispenser: 'distributor-portal.html?view=dispenser',
        ...
        async function completeDistributorLogin(password) {
            ...
            try {
                localStorage.setItem('currentUser', JSON.stringify(distributorUser));
                localStorage.setItem('currentUserRole', 'client');
            } catch (storageError) {
                console.warn('Unable to persist distributor session user.', storageError);
            }
            ...
            const finalUrl = needsNb
                ? (targetUrl.includes('nb=') ? targetUrl : `${targetUrl}${targetUrl.includes('?') ? '&' : '?'}nb=${encodeURIComponent(normalizedCode)}`)
        ...
        function openTab(tabName, event) {
            ...
            if (tabName === 'client') {
                showDistributorTab('shop');
                document.getElementById('distributorSubTabs')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
                return;
            }
        ...
        function showDistributorTab(tabName, evt) {
            ...
            document.querySelectorAll('.distributor-tab-btn').forEach(btn => btn.classList.remove('active'));
            ...
            try {
                ...
                if (testimonials.length === 0) {
                    const defaultTestimonials = [
                        { id: 'TEST001', name: 'Sarah M.', location: 'Windhoek, Namibia', text: 'Two years ago, I was dying… today, I am healthy and 6 months pregnant! Dynapharm products have truly transformed my life. I am so grateful for the natural solutions they provide.', date: new Date().toISOString(), approved: true },
                        { id: 'TEST002', name: 'Maria K.', location: 'Ondangwa, Namibia', text: 'I had cancer of the ovaries stage 3… I have an option to treat it by taking Dynapharm products… The natural approach combined with my medical treatment has been life-changing. I feel stronger and more hopeful every day.', date: new Date().toISOString(), approved: true },
                        { id: 'TEST003', name: 'John D.', location: 'Swakopmund, Namibia', text: 'My arthritic pain was unbearable… After using Dynapharm\'s natural relief products for just a few weeks, I noticed a significant improvement. I can now move freely without constant pain.', date: new Date().toISOString(), approved: true },
                        ...
                        { id: 'TEST005', name: 'Grace L.', location: 'Katima Mulilo, Namibia', text: 'I HAD BREAST CANCER …NOW I AM HEALED! Dynapharm products have been an essential part of my healing journey. The support and quality products have given me hope and strength.', date: new Date().toISOString(), approved: true }
                    ];
        ...
        function openLandingPageEditor() {
            ...
            document.getElementById('editHeroBadge').value = content.heroBadge || '🏥 Trusted Health Partner Since 2010';
